...
function manDist(u, v) {
  return Math.abs(u.x - v.x) + Math.abs(u.y - v.y)
}

// 0 == 'vertical'
// 1 == 'horizontal'
function getOrientation(edge) {
...
// checks if edge intersects with vertical line at x
function verticalIntersect(edge, x) {
  return edge.v1.x < x && x < edge.v2.x
}

// check if edge intersects with horizontal line at y
function horizontalIntersect(edge, y) {
  return edge.v1.y < y && y < edge.v2.y
}

function getIntersection(edgeA, edgeB) {
  if (getOrientation(edgeA) === getOrientation(edgeB)) {
    return null
  }

  const verticalEdge = getOrientation(edgeA) === 0 ? edgeA : edgeB
  const horizontalEdge = getOrientation(edgeA) === 1 ? edgeA : edgeB

  if (horizontalIntersect(verticalEdge, horizontalEdge.v1.y)
      && verticalIntersect(horizontalEdge, verticalEdge.v1.x)) {
    return {x: verticalEdge.v1.x, y: horizontalEdge.v1.y}
  }
  return null
}

function makeOrderedEdge(u, v) {
  if (getOrientation({ v1: u, v2: v }) === 0) {
    // edge is vertical
    const
      v1 = Math.min(u.y, v.y) == u.y ? u : v
      v2 = Math.max(u.y, v.y) == u.y ? u : v
    return { v1, v2 }
  }

  // edge is horizontal
  const
    v1 = Math.min(u.x, v.x) == u.x ? u : v
    v2 = Math.max(u.x, v.x) == u.x ? u : v
  return { v1, v2 }
}

function solve(inA, inB) {
  const
    pathA = toPoints(inA),
    pathB = toPoints(inB)
  let
  ...
  for (let i = 0; i < pathA.length - 1; i++) {
    const edgeA = makeOrderedEdge(pathA[i], pathA[i+1])
    for (let j = 0; j < pathB.length - 1; j++) {
      const edgeB = makeOrderedEdge(pathB[j], pathB[j+1])

      const intersection = getIntersection(edgeA, edgeB)
      if (intersection !== null) {
        let
          deltaA = NaN,
          deltaB = NaN
        if (getOrientation(edgeA) === 1) {
          // edgeA is horizontal, edgeB is vertical
          deltaA = Math.abs(pathA[i].x - intersection.x)
          deltaB = Math.abs(pathB[j].y - intersection.y)
        } else {
          // edgeA is vertical, edgeB is horizontal
          deltaA = Math.abs(pathA[i].y - intersection.y)
          deltaB = Math.abs(pathB[j].x - intersection.x)
        }
        const dist = pathA[i].steps + pathB[j].steps + deltaA + deltaB
        if (dist < bestDist) {
          bestDist = dist
...
console.log(solve(inputA, inputB))